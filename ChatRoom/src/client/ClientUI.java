...
public class ClientUI extends JFrame implements Event {
	...
	button.addActionListener(new ActionListener() {
		...
		if (name != null && name.length() > 0) {
		    SocketClient2.setUsername(name);
		    self.next();
		}
	    }
	...
	button.addActionListener(new ActionListener() {
		...
		if (text.getText().length() > 0) {
		    SocketClient2.sendMessage(text.getText());
		    text.setText("");
		}
	    }
    ...
    void color(Graphics message) {
    	message.setColor(Color.red); 
    }
    
    void bold (Graphics message) {
   Font myFont = new Font("Courier", Font.BOLD ,20);
    	message.setFont(myFont);
    }
    
    void ital (Graphics message) {
    	Font myFont = new Font("Courier", Font.ITALIC ,20);
    	message.setFont(myFont);
    }
    
    void under (Graphics message) {
    	message.drawLine(5,5,5,5);
    }
	...
	SocketClient2.callbackListener(this);
	SocketClient2.connectAndStart(host, port);
    }
